...
\section{Quicksort} % TO DO: use fig.~\ref{}, listing~\ref{}, tab.~\ref{}
\label{sec: QuickSortImp}
Quicksort is an in-place sorting algorithm, developed by British computer scientist Tony Hoare in 1959. It is a divide-and-conquer algorithm. It works by selecting a 'pivot' and dividing a large array into two sub-arrays of low and high elements, depending if they are bigger or smaller than the pivot. Recursive algorithms, especially ones based on a form of divide-and-conquer , are a great candidate for parallelization and CPU-bound computations. Quicksort pseudocode (shown in listing~\ref{lst:PseudoQuickSort}) is based on description from \emph{Introduction to Algorithms}~\cite{Cormen2009}.
...
% TO DO: if line numbering is not needed, then remove it
...
% TO DO: check which is correct: "on figure" or "in figure"
